...
- [x] **gpuPickedPointList memory leak** — resolved by removing GPU pick path from `getPickedPoint`/`getPickedPointList` entirely. Hover preselection uses `gpuPick` + `resolveGpuPickIdentity` directly in `SoFCUnifiedSelection`, bypassing `getPickedPoint`. Mouse button events use ray pick for dragger deep paths. No more `SoPickedPoint` objects with stale `SoPath` references.
