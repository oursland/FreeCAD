...
- [x] **Proper command path lifecycle** — resolved: `storeCommandPath`/`getCommandPath` are stubbed out (no-ops). Unsafe `SoPath` storage removed. Hover uses string-based `resolveGpuPickIdentity`, click uses `pickIdentity` prefix matching.
- [ ] **gpuPickedPointList memory leak** — `SoHandleEventAction::getPickedPoint()` creates `SoPickedPoint` objects from GPU pick results and appends them to `gpuPickedPointList`. The list is cleared via `SbPList::truncate(0)` which does NOT delete the objects — it just resets the list pointer. Using `unref()`/`delete` crashes because the picked points contain `SoPath` copies built from stale command paths (`unrefNoDelete` pointers). This leak accumulates over time and may contribute to dragger interaction instability — the stale `SoPickedPoint` objects contain dangling node references that could be accessed by Coin's internal pick caching. Fix: either properly ref-count the paths or avoid creating `SoPickedPoint` entirely (extend the string-based identity approach to all pick consumers).
- [x] **Sync public/internal IR headers** — CMake POST_BUILD command copies `src/rendering/SoModernIR.h` to `include/Inventor/rendering/SoModernIR.h` automatically. Single source of truth, no manual sync needed.
